...
void CJOCh264bitstream(FILE *pOutBinaryFile)
{
	clearbuffer();
...
void addbytetostream (int nVal)
{
	...
	if (nBitPosInByte != 7){
		printf("Error: inserting not aligment byte");
		return;
	}
...
void savebufferbyte(bool bemulationprevention)
{
	...
	if ((m_nLastbitinbuffer % 8)  != 0){
		printf("Error: Save to file must be byte aligned");
	}

	if ((m_nLastbitinbuffer / 8) <= 0){
		printf("Error: NO bytes to save");
	}
...
void addbits (unsigned long lval, int nNumbits)
{
	if ((nNumbits <= 0 )||(nNumbits > 64)){
		printf("Error: numbits must be between 1 ... 64");
		return;
	}
...
void add4bytesnoemulationprevention (unsigned int nVal, bool bDoAlign)
{
