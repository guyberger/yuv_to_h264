...
//Contructor
void CJOCh264encoder(FILE *pOutFile)
{
	CJOCh264bitstream(pOutFile);
	m_lNumFramesAdded = 0;
...
{
	if (m_frame.yuv420pframe.pYCbCr != NULL){
		printf("Error: null values in frame");
		return;
	}
...
{
	add4bytesnoemulationprevention (0x000001, false); // NAL header
...
void IniCoder (int nImW, int nImH, int nImFps, enSampleFormat SampleFormat, int nSARw, int nSARh)
{
	...
	if (SampleFormat != SAMPLE_FORMAT_YUV420p){
		printf("Error: SAMPLE FORMAT not allowed. Only yuv420p is allowed in this version");
		return;
	}
	...
	if (SampleFormat == SAMPLE_FORMAT_YUV420p)
	{
		...
		if (((nImW % MACROBLOCK_Y_WIDTH) != 0)||((nImH % MACROBLOCK_Y_HEIGHT) != 0)){
			printf("Error: size not allowed. Only multiples of macroblock are allowed (macroblock size is: 16x16)");
			return;
		}
	}
	m_nFps = nImFps;
...
{
	if (m_frame.yuv420pframe.pYCbCr == NULL){
		printf("Error: video frame is null (not initialized)");
		return NULL;
	}
